...
public class ContactController {
    ...
    public ResponseEntity findById(@PathVariable long id){
        return repository.findById(id)
                .map(record -> ResponseEntity.ok().body(record))//se encontrar retorna status 200 ok no corpo da resposta
                .orElse(ResponseEntity.notFound().build());// caso nao encontre retorna status 404 nao encontrado.
    }
